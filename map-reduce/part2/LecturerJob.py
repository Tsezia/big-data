...
class LecturerJob(MRJob):
    ...
    def lecturer_name_to_id_mapper(self, lesson, lecturer):
        ...
        link = 'https://rasp.omgtu.ru/api/search?term={}&type=person'.format(quote(lecturer))
    ...
    def lecturer_id_to_weekly_schedule_mapper(self, lesson, lecturer_id):
        ...
        while start_time != datetime(2024,1,1):
            link = "https://rasp.omgtu.ru/api/schedule/person/{}?start={}&finish={}&lng=1".format(lecturer_id, start_time.strftime('%Y.%m.%d'), (start_time + timedelta(days=5)).strftime('%Y.%m.%d'))
            response = requests.get(link, verify=True)
            if response.status_code == 200:
