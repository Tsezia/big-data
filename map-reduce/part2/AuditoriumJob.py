...
class Rescheduling(MRJob):
    ...
    def auditoriums_name_to_auditorium_id_mapper(self, _, auditorium):
        link = "https://rasp.omgtu.ru/api/search?term={}&type=auditorium".format(auditorium)
    ...
    def auditoriums_id_to_auditorium_weekly_schedule_mapper(self, auditorium_name, auditorium_id):
        ...
        while start_time != datetime(2024,1,1):
            link = "https://rasp.omgtu.ru/api/schedule/auditorium/{}/?start={}&finish={}&lng=1".format(auditorium_id, start_time.strftime('%Y.%m.%d'), (start_time + timedelta(days=5)).strftime('%Y.%m.%d'))
            response = requests.get(link, verify=True)
            if response.status_code == 200:
                weekly_schedule = json.loads(response.text)
                weekly_schedules["{} - {}".format(start_time.strftime('%Y.%m.%d'), (start_time + timedelta(days=5)).strftime('%Y.%m.%d'))] = weekly_schedule
            else:
